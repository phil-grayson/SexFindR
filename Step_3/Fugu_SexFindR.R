...
gemma_window_count_fugu <- read_tsv("~/Desktop/fugu/gemma_window_count.txt") %>% arrange(-count) %>% rownames_to_column() %>% rename(GWAScount_rank=rowname) %>% mutate(GWAScount_rank = as.numeric(GWAScount_rank))
...
# now we want Fst
Fstugu <- read_tsv("~/Desktop/fugu/biallelic_fst.weir.fst") %>% replace_na(list(WEIR_AND_COCKERHAM_FST=0)) %>% rename(scaf = CHROM, base=POS) %>% mutate(base=as.numeric(base)) %>% filter(WEIR_AND_COCKERHAM_FST >=0) %>% filter(grepl("NC_",scaf)) %>% filter(scaf != "NC_004299.1")
...
Fstugu_window_count <- read_tsv("~/Desktop/fugu/Fstugu_window_count.txt") %>% arrange(-count) %>% rownames_to_column() %>% rename(Fstcount_rank=rowname) %>% mutate(Fstcount_rank = as.numeric(Fstcount_rank))
