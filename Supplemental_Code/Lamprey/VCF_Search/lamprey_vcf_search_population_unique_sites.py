#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 19 16:52:49 2020

@author: phil
"""

# execute with filtered_PASS_sea_lamprey_304_update.vcf as $1, outfile as $2, site as $3

# options for $3 are Erie, Huron, Michigan, Ontario, Superior, Cayuga, Phil, Holyoke, Seneca (but we won't do Seneca since there is only one female)

import csv
import sys

waters = dict(Erie = ["BC","GO"],Huron = ["Augres","BL","CH","SM"],Michigan = ["GR","MAN"],Ontario = ["BLR","DF"],Superior = ["BR","RO","TQ"],Cayuga = ["NY"],Phil = ["d_"],Holyoke = ["CTR"],Seneca = ["CC"])


# get male names to index from the header of vcf
males=[]
females=[]
with open("sexed_update_for_python.txt") as handle:
    reader=csv.reader(handle,delimiter='\t')
    water = (sys.argv[3])
    population=waters[water]
    for strLine in reader:
        for code in population:
            if code in strLine[0]:
                if strLine[1] == "M":
                    males.append(strLine[0])
                if strLine[1] == "F":
                    females.append(strLine[0])

fileout=(sys.argv[3]+"_unique_"+sys.argv[2])

fout= open(fileout, 'w')
fout.write("chrom"+"\t"+"base"+"\t"+"male_geno"+"\t"+"female_geno"+"\t"+"male_number"+"\t"+"female_number"+"\t"+"male_unique"+"\t"+"female_unique"+"\n")

with open(sys.argv[1]) as handle:
    reader=csv.reader(handle,delimiter='\t')   
    for strLine in reader:
        if len(strLine) > 1:
            if strLine[0] == "#CHROM":
                male_indexes=[]
                for male in males:
                    x=-1
                    for meta in strLine:
                        x+=1
                        if male+"_1" in meta:
                            male_indexes.append(x)
                female_indexes=[]
                for female in females:
                    x=-1
                    for meta in strLine:
                        x+=1
                        if female+"_1" in meta:
                            female_indexes.append(x)
                print(male_indexes)
                print(female_indexes)
            else:
                male_genotype = []
                female_genotype = []
                for index in male_indexes:
                    male_genotype.append(strLine[index].split(":")[0])
                for index in female_indexes:
                    female_genotype.append(strLine[index].split(":")[0])
                if set(male_genotype) != set(female_genotype):
                    male_dict={}
                    female_dict={}
                    for genotype in set(male_genotype):
                        male_dict[genotype]=male_genotype.count(genotype)
                    for genotype in set(female_genotype):
                        female_dict[genotype]=female_genotype.count(genotype)
                    male_singles=[]
                    female_singles=[]
                    for key in male_dict.keys():
                        if male_dict[key] == 1:
                            male_singles.append(key)
                    for key in male_singles:
                        del male_dict[key]
                    for key in female_dict.keys():
                        if female_dict[key] == 1:
                            female_singles.append(key)
                    for key in female_singles:
                        del female_dict[key]
                    if './.' in male_dict.keys():
                        del male_dict['./.']
                    if './.' in female_dict.keys():
                        del female_dict['./.']
                    if male_dict.keys() != female_dict.keys():
                        male_unique = []
                        female_unique = []
                        for key in male_dict.keys():
                            if key not in female_dict.keys():
                                male_unique.append(key)
                        for key in female_dict.keys():
                            if key not in male_dict.keys():
                                female_unique.append(key)
                        fout.write(strLine[0]+"\t"+strLine[1]+"\t"+str(male_dict)+"\t"+str(female_dict)+"\t"+str(len(male_dict))+"\t"+str(len(female_dict))+"\t"+str(len(male_unique))+"\t"+str(len(female_unique))+"\n")
fout.close()
